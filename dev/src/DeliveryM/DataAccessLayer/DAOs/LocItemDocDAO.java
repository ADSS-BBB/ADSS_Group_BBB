...
public class LocItemDocDAO extends DataDAO{
    ...
    public void deleteAllDocs() throws SQLException {
        this.connection = connect();
        String sql = "DELETE FROM LocItemDocs";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e.getMessage());
    ...
    public void updateWeight(int targetDeliveryId, int newWeight) {
        List<LocItemDocDTO> locdocs = new ArrayList<>();
        this.connection = connect();
        String selectSql = "SELECT * FROM LocItemDocs WHERE deliveryId = ?";
        String updateSql = "UPDATE LocItemDocs SET currecntTruckWeight = ? WHERE deliveryId = ?";

        try (PreparedStatement selectStmt = connection.prepareStatement(selectSql);
             PreparedStatement updateStmt = connection.prepareStatement(updateSql)) {

            // Update currentTruckWeight
            updateStmt.setInt(1, newWeight);
            updateStmt.setInt(2, targetDeliveryId);
            updateStmt.executeUpdate();

            // Select updated records
            selectStmt.setInt(1, targetDeliveryId);
            ...
            while (rs.next()) {
                ...
                int docid = rs.getInt("docid");
                String locid = rs.getString("locid");
                int currentTruckWeight = rs.getInt("currecntTruckWeight");
                int Qid = rs.getInt("Qid");
                int driverId = rs.getInt("driverId");
                LocItemDocDTO toAdd = new LocItemDocDTO(deliveryId, docid, locid, currentTruckWeight, Qid, driverId);
                locdocs.add(toAdd);
            }
        } catch (SQLException e) {
            System.out.println(e.getMessage());
